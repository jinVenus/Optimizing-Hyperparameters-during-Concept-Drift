...
dtype = torch.double
folder_name = 'boost_mnist_continue'
...
def call_external(X, num_trial, num_param, dataset):
    ...
    for i in range(0, num_trial):
        ...
        commands = 'python E:\\ML\\Hiwi\\src\\algorithms\\botorch_modes\\boost\\call_ml_idv.py' + ' -i ' + str(
...
def generate_initial_data(dataset):
    ...
    exact_obj = call_external(train_x, 30, 6, dataset)
...
BATCH_SIZE = 1
bounds = torch.tensor([[0, 2, 0, 1, 0, 0], [199.99, 20.99, 40.99, 20.99, 1.99, 11.99]], device=device, dtype=dtype)
...
def generate_rand(sets):
    ...
    for i in range(0, sets):
        data = []
        data.append(random.randint(0, 199))
...
def optimize_acqf_and_get_observation(acq_func, dataset):
    ...
    exact_obj = call_external(new_x, BATCH_SIZE, 6, dataset).unsqueeze(-1)  # add output dimension
    new_obj = exact_obj
