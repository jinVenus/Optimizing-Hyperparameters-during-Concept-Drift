...
def call_external(X, num_trial, num_param, dataset):
    ...
    for i in range(0, num_trial):
        ...
        commands = 'python E:\\ML\\Hiwi\\src\\algorithms\\botorch_modes\\mlp\\call_ml_idv.py' + ' -i ' + str(
        ...
        sum_result = float(sorted_results[1][0])
...
def generate_initial_data(dataset):
    ...
    train_x = generate_rand(36)
    exact_obj = call_external(train_x, 36, 5, dataset)
...
def main(argv):
    ...
    for trial in range(2, N_TRIALS + 1):
        ...
        csv_file_name = 'E:\\ML\\Hiwi\\results\\' + 'hp-gp-ei-dataset-' + str(
